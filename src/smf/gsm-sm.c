...
void smf_gsm_state_operational(ogs_fsm_t *s, smf_event_t *e)
{
            ...
            CASE(OGS_SBI_RESOURCE_NAME_SM_POLICIES)
                if (!sbi_message->h.resource.component[1]) {
                    ...
                    CASE(OGS_SBI_RESOURCE_NAME_DELETE)
                        ...
                        // 用于测试PCF触发的PDU release释放目的，这里源码是_SMF_INITIATED，改成了_PCF_INITIATED
                        if (state == OGS_PFCP_DELETE_TRIGGER_PCF_INITIATED) {
     ...
     * Related Issue #2396
     */
                            // 用于测试PCF触发的PDU release释放目的，非正常流程；n1n2和PFCP删除
                            smf_namf_comm_send_n1_n2_pdu_establishment_reject(sess);
                            smf_5gc_pfcp_send_session_deletion_request(sess, stream, state);
                            // 如下为源码，用于SMF触发的场景，注释掉
                            //smf_namf_comm_send_n1_n2_message_transfer(sess, &param);
                        } else {
