        ...
        smf_sess_t *sess, void *data)
{
    ...
    /*add location to SmPolicyContextData by Geng*/
    OpenAPI_user_location_t ue_location;
    OpenAPI_nr_location_t nr_location;
    memset(&ue_location, 0, sizeof(ue_location));
    memset(&nr_location, 0, sizeof(nr_location));

    nr_location.tai = ogs_calloc(1, sizeof(OpenAPI_tai_t));
    nr_location.tai->plmn_id = ogs_calloc(1, sizeof(OpenAPI_plmn_id_t));
    nr_location.tai->plmn_id->mcc = ogs_msprintf("%03d", ogs_plmn_id_mcc(&sess->nr_tai.plmn_id));
    nr_location.tai->plmn_id->mnc = ogs_msprintf("%02d", ogs_plmn_id_mnc(&sess->nr_tai.plmn_id));
    nr_location.tai->tac = ogs_uint24_to_0string(sess->nr_tai.tac);

    nr_location.ncgi = ogs_calloc(1, sizeof(OpenAPI_ncgi_t));
    nr_location.ncgi->plmn_id = ogs_calloc(1, sizeof(OpenAPI_plmn_id_t));
    nr_location.ncgi->plmn_id->mcc = ogs_msprintf("%03d", ogs_plmn_id_mcc(&sess->nr_cgi.plmn_id));
    nr_location.ncgi->plmn_id->mnc = ogs_msprintf("%02d", ogs_plmn_id_mnc(&sess->nr_cgi.plmn_id));
    nr_location.ncgi->nr_cell_id = ogs_msprintf("%09x", (uint32_t)sess->nr_cgi.cell_id); // 转换为 uint32_t

    ue_location.nr_location = &nr_location;
    ue_location.nr_location->ue_location_timestamp = ogs_sbi_gmtime_string(sess->ue_location_timestamp);

    SmPolicyContextData.user_location_info = &ue_location;
    // end add location
    ...
    /*release mem added by Geng*/
    if (nr_location.tai)
    {
        if (nr_location.tai->plmn_id)
        {
            ogs_free(nr_location.tai->plmn_id->mcc);
            ogs_free(nr_location.tai->plmn_id->mnc);
            ogs_free(nr_location.tai->plmn_id);
        }
        ogs_free(nr_location.tai->tac);
        ogs_free(nr_location.tai);
    }

    if (nr_location.ncgi)
    {
        if (nr_location.ncgi->plmn_id)
        {
            ogs_free(nr_location.ncgi->plmn_id->mcc);
            ogs_free(nr_location.ncgi->plmn_id->mnc);
            ogs_free(nr_location.ncgi->plmn_id);
        }
        ogs_free(nr_location.ncgi->nr_cell_id);
        ogs_free(nr_location.ncgi);
    }

    if (ue_location.nr_location)
    {
        if (ue_location.nr_location->ue_location_timestamp)
        {
            ogs_free(ue_location.nr_location->ue_location_timestamp);
        }
    }
    /*end of the mem release*/
