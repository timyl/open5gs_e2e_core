        ...
        smf_sess_t *sess, OpenAPI_sm_policy_decision_t *SmPolicyDecision)
{
    ...
    if (SmPolicyDecision->pcc_rules) {
        ...
        OpenAPI_list_for_each(SmPolicyDecision->pcc_rules, node) {
            ...
            ogs_assert(pcc_rule->id);
            /* Map precedence to valid range (1-254) */
            if (PccRule->precedence > 0) {
                /* If precedence is out of range, map it to 1-254 */
                if (PccRule->precedence >= 255) {
                    /* Map to 1-254 range, keeping relative priority */
                    pcc_rule->precedence = 1 + ((PccRule->precedence - 1) % 253);
                    ogs_warn("PCC Rule precedence %d mapped to %d to fit valid range (1-254)", 
                            PccRule->precedence, pcc_rule->precedence);
                } else {
                    pcc_rule->precedence = PccRule->precedence;
                }
            } else {
                /* If precedence is 0 or negative, use default value */
                pcc_rule->precedence = 254;
                ogs_warn("Invalid PCC Rule precedence %d, using default value 254", 
                        PccRule->precedence);
            }
            ...
            if (PccRule->flow_infos) {
                ...
                OpenAPI_list_for_each(PccRule->flow_infos, node2) {
                    ...
                    /* Skip empty flow descriptions to prevent parsing errors */
                    if (strlen(flow->description) == 0) {
                        ogs_warn("Empty flow description received from PCF, skipping this flow");
